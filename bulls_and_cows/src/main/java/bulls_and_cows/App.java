...
public class App {
    ...
    private static String runArguments(final String[] args) {
        ...
        if (args.length == 3 && args[0].matches("^(-g|--game)$")) {
            ...
            if (!modeString.matches("^([diu]|distinct|indistinct|userinput)?$")) {
                console.format(
                    "命令行参数有误\n应为以下中的一项 [d|i|u] 或 [distinct|indistinct|userinput]\n输入为：%s",
                    modeString
                );
                System.exit(1);
                return null;
        ...
        length = Integer.parseInt(lengthString.length() == 0 ? "4" : lengthString);
        switch (modeString) {
            default:
            case "d":
            case "distinct":
        ...
        return App.getSecret(length, mode);
    }
    ...
    private static void runLoop(final String secret, final Boolean autoQuit) {
        ...
        if (Boolean.TRUE.equals(autoQuit) || console.readLine("继续下一轮猜测吗[y]n：").equals("n")) {
            System.exit(0);
            return;
        }
        App.runLoop(App.runArguments(new String[0]), false);
    }
        ...
        switch (mode) {
            ...
            case INDISTINCT:
                ...
                while (secretBuilder.length() < length)
                    secretBuilder.append((Integer.valueOf(random.nextInt(10))).toString());
